...
const Rekomendasi = () => {
    ...
    const columns: ColumnType<SearchRekomendasiRespons[0]>[] = [
        ...
        },
        {
            ...
            render: (_value, row) => {
                if (row.UserRekomendasiKainstal.length == 0) {
                    return <Tag color='red'>Belum</Tag>
                } else {
                    const rec = row.UserRekomendasiKainstal[0]
                    if (rec.status_approval == "") {
                        return <Tag color='red'>Belum</Tag>
                    } else {
                        return <Tag color={`${rec.status_approval.includes("tidak") ? "red" : "green"}`}>{rec.status_approval}</Tag>
                    }
                }
            }
        },
            ...
            render: (_text: string, record) => (
                <Space size="small">
                    <Button icon={<EditOutlined />} onClick={() => {
                        navigate({ to: "/kakomwat/rekomendasi/penilaian/$id", params: { id: record.id } })
                    }} />
                </Space>
            ),
        },
...
export const Route = createFileRoute('/__guard/kakomwat/rekomendasi/')({
