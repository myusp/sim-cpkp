...
import { detailRekomendasiByIdPenilaian, detailRekomendasiByIdPenilaianResponse, submitFeedbackRekomendasiByIdPenilaian } from '@/services/rekomendasi'
...
const DetailPenilaian = () => {
    ...
    const submitFeedback = () => {
        ...
        if (feedback === "") {
            message.error({ content: "Harap isi feedback" })
            return
        }
        loadingHandler.open()
        submitFeedbackRekomendasiByIdPenilaian({
            feedback,
            penilaianId: id,
            user: user?.email || "",
        }).then(res => {
    ...
    return <>
        ...
        <div className="p-4 bg-white rounded-lg shadow-md">
            <Row gutter={[8, 8]}>
                ...
                <Col span={24} md={14}>
                    <KainstalFeedbackDropdown disabled={data?.rekomendasi != null} value={feedback} placeholder="Feedback untuk karu" className='w-full' onChange={e => setFeedback(e)} />
                </Col>

                <Col span={24} className='mt-4'>
                    <Flex justify='right'>
                        <Button disabled={loading} className='mr-2' onClick={() => navigate({ to: "/kainstal/feedback" })}>Kembali</Button>
                        <Button disabled={data?.rekomendasi != null} loading={loading} onClick={submitFeedback} icon={<SendOutlined />} type='primary'>Simpan Feedback</Button>
                    </Flex>
                </Col>
...
export const Route = createFileRoute('/__guard/kainstal/feedback/penilaian/$id')({
