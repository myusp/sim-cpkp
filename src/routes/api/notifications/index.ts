...
const notification: FastifyPluginAsync = async (fastify, opts): Promise<void> => {
    ...
    // Get all notifications for a specific karu ()
    fastify.get<{ Querystring: { toKaruEmail: string } }>('/getlistnotif-karu',
        // { onRequest: [fastify.authenticate] },
        async (request, reply) => {
            try {
                const { toKaruEmail } = request.query;
                const notifications = await fastify.prisma.notificationToKaru.findMany({
                    where: { toKaruEmail: toKaruEmail },
                    orderBy: { createdAt: 'desc' },
                    select: {
                        perawatEmails: true,
                        selfAsesmenDate: true,
    ...
    // Get unread notification count for a specific nurse (toPerawatEmail)
    fastify.get<{ Querystring: { toKaruEmail: string } }>('/getcountnotif-karu', { onRequest: [fastify.authenticate] }, async (request, reply) => {
        try {
            const { toKaruEmail } = request.query;
            const count = await fastify.prisma.notificationToKaru.count({
                where: { toKaruEmail, isRead: false },
            });
            return { count };
        } catch (error) {
            reply.status(500);
    ...
    // Mark a notification as read by notification ID
    fastify.put<{ Params: { id: string } }>('/readnotif-karu/:id', { onRequest: [fastify.authenticate] }, async (request, reply) => {
        try {
            const { id } = request.params;
            await fastify.prisma.notificationToKaru.update({
                where: { id },
                data: { isRead: true },
            });
            return { message: "successs" };
